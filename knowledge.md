...
- **three r185's TSL build has NO chainable bloom/ssao/fxaa nodes** (verified in `three.tsl.js` exports) — the spec's `pass().pipe(bloom).pipe(fxaa)` is unavailable until a three upgrade. Task 3.5 ships dual path: WebGL `three/addons` EffectComposer with a TSL-safe RenderPass + OutputPass fallback and guarded UnrealBloomPass/FXAA for classic-material scenes, plus WebGPU `PostProcessing` with runtime feature-detection of `passNode.bloom()`.
...
- **SDXL-Turbo fp16 on RTX 2070 (RESOLVED):** the 1-step generation "hang" was allocator thrash, not a kernel bug. The bare fp16 UNet forward completes in 0.3 s at 5.0 GB peak; the full-resident pipeline (~6.7 GB) thrashes at 7.5/8 GB (cuDNN workspace can't fit → 100% GPU forever). Production's `enable_model_cpu_offload()` is correct and fast (0 MB before gen, 5.1 GB peak, ~6–10 s). Two real fixes shipped: (1) diffusers 0.31 `callback_on_step_end` must return kwargs; (2) the live-battery "hang" was orphaned backend processes holding 7.7 GB — always check `nvidia-smi` before judging. Battery script bugs fixed: cp1252 `≈` crash, WS jobId race (server publishes DURING the POST — events precede the response, so buffer until the jobId is known), 2-tuple tally.

- **SmolVLM real path (transformers 4.46):** there is NO `image-text-to-text` pipeline task in 4.46. Drive `AutoProcessor` + `AutoModelForImageTextToText` directly with the SmolVLM chat template (`apply_chat_template` + `processor(text=..., images=[...])` + `model.generate`), decode with `processor.batch_decode(outputs[:, inputs["input_ids"].shape[1]:])` (BatchFeature is a dict — index, don't use attributes). Needs `qwen-vl-utils` installed (now in requirements.txt). Verified live on CUDA: ~9s first call incl. 5.6 GB load.
- **Real-GPU WebGPU verification (3 non-obvious launch requirements):** use `channel: 'chrome'` (Playwright's bundled Chromium exposes NO WebGPU adapter), headed mode (headless has none — three r185's WebGPURenderer then SILENTLY falls back to WebGL2, so a pixel test passes for the wrong reason), and NO `--enable-features=Vulkan,DefaultANGLEVulkan` (that flag makes Dawn try Vulkan, fail, and `requestAdapter()` return null even though D3D12 works). Check: `scripts/gpu/webgpu-check.mjs` against `NEXT_PUBLIC_ENABLE_WEBGPU=true pnpm dev -p 3100`.
- **three r185 WebGPU: classic ShaderMaterials are NOT compatible** (`NodeBuilder: Material "ShaderMaterial" is not compatible` → silently replaced with an empty NodeMaterial). drei's `<Grid>` is a ShaderMaterial and disappears on WebGPU. Use TSL node materials instead — the app's `GridFloor.tsx` is the pattern (world-position line pattern via `positionWorld.x`, `fract`/`min`/`smoothstep`).
