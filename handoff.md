...
- **3.5 Post-FX (dual path)** — WebGL keeps a TSL-safe RenderPass+OutputPass fallback and guards UnrealBloomPass+FXAA for classic-material scenes; WebGPU TSL path feature-detects bloom — **three r185 TSL ships no bloom/fxaa nodes**, so it renders the plain pass until a three upgrade (knowledge.md).
...
## Work completed (2026-08-26, resumed session — SmolVLM real path + WebGPU real-GPU verification)

- **Real SmolVLM vision now works** (user authorized `qwen-vl-utils` install, `backend/requirements.txt`). Two real bugs fixed in `smolvlm_service.py`: (1) transformers 4.46 has NO `image-text-to-text` pipeline task — switched to the canonical processor + `AutoModelForImageTextToText` with the SmolVLM chat template (verified live on CUDA: `synthetic: false`, "It's dark out." on a generated campfire frame, ~9s first call incl. 5.6 GB model load, slot unloaded after); (2) `inputs.input_ids` → `inputs["input_ids"]` (BatchFeature is a dict). Added 4 hermetic unit tests (fake processor/model). **Backend: 68 tests green.** Real-path note: `GPU_TIMEOUT_SMOLVLM=120` default covers the ~9s first call with margin.
- **WebGPU path verified on the REAL GPU** (real Chrome, RTX 2070): `scripts/gpu/webgpu-check.mjs` (tracked) — **7/7 checks**: `nvidia/turing` hardware adapter, viewport canvas is a WebGPU context, zero page errors, **163 FPS**, FX toggle, TSL post path engaged (three r185 has no chainable bloom/fxaa — plain pass, documented). **Two empirical launch requirements** (both non-obvious): must use the user's Chrome (`channel: 'chrome'` — Playwright's bundled Chromium exposes NO WebGPU adapter at all) and must be headed (headless has no adapter → three's r185 WebGPURenderer silently falls back to WebGL2 and the check would pass for the wrong reason). And **the `--enable-features=Vulkan,DefaultANGLEVulkan` flag BREAKS adapter selection** on Windows (Dawn tries Vulkan, fails, `requestAdapter()` → null — no flags needed, Chrome enables WebGPU by default on localhost).
