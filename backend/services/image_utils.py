...
model); to keep the backend dependency-light and the first-run experience
instant, we ship a small dependency-free heuristic instead:

* ``decode_image`` — base64 data-URL → PIL image (with size guards).
* ``prepare_foreground`` — center-crop to square, estimate a foreground alpha
  mask with a border flood-fill seeded from the four edges, feather it, and
  composite onto a white canvas. Alpha-bearing uploads keep their own alpha.

The flood-fill runs on a 256 px proxy and the mask is upsampled, so it costs
well under 100 ms for typical uploads. The endpoint contract stays identical,
so swapping in ``rembg`` later is a drop-in replacement.
"""
...
def prepare_foreground(image: Image.Image, size: int = 512) -> Image.Image:
    ...
    #    exist, otherwise estimate via border flood-fill on a small proxy.
    alpha = np.asarray(image)[..., 3]
    if alpha.min() < 250:
        # Upload already has meaningful transparency.
        pass
    else:
